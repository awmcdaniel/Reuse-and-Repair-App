...
$app->post('/api/organizationitem', function () use ($app, $db) {
    ...
    if ($result_org && $result_item) {
        ...
        if (!$result_org) {
            $msg[] = "OrganizationItem ID=$organization_id does not exist in DB. Use POST /api/organizations to add organizations.";
        }

        if (!$result_item) {
            $msg[] = "OrganizationItem ID=$item_id does not exist in DB. USE POST /api/item to add items.";
        }
