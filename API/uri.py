...
class URI(webapp2.RequestHandler):
	def get(self):
		...
		self.response.write('<ul>')	
		self.response.write('<li>reuse_org_urlsafe_key - the urlsafe key of the Reuse_org entity to be updated, REQUIRED</li>')
		self.response.write('<li>name - stored as a StringProperty, REQUIRED</li>')
		self.response.write('<li>address - stored as a TextProperty</li>')
		self.response.write('<li>longitude - stored as a StringProperty</li>')
		self.response.write('<li>latitude - stored as a StringProperty</li>')
		...
		self.response.write('<li>notes - stored as a TextProperty</li>')
		self.response.write('<li>items - stored as a list of KeyProperties (format is a urlsafe key)</li>')
		self.response.write('<li>updated_by - stored as a StringProperty</li>')
		self.response.write('</ul>')	
		self.response.write('</ul>')	

		# /reuse_org/delete/<reuse_org_urlsafe_key>
		self.response.write('<li><h2>')
		self.response.write(cgi.escape('/reuse_org/delete/<reuse_org_urlsafe_key>'))
		...
		self.response.write('<ul>')	
		self.response.write('<li>reuse_org_urlsafe_key - The urlsafe key of the Reuse_org entity</li>')		
		self.response.write('</ul>')					
		self.response.write('</ul>')	
		
		# /reuse_org/search_by_item/<reuse_item_urlsafe_key>
		self.response.write('<li><h2>')
		self.response.write(cgi.escape('/reuse_org/search_by_item/<reuse_item_urlsafe_key>'))
		self.response.write('</h2></li>')
		self.response.write('<ul>')
		self.response.write('<li><h4>HTTP GET returns JSON object with all Reuse_org entities (in alphabetical order) which have a key reference to the queried Reuse_item entity</h4></li>')
		self.response.write('<ul>')	
		self.response.write('<li>reuse_item_urlsafe_key - The urlsafe key of the Reuse_item entity</li>')		
		self.response.write('</ul>')					
		self.response.write('</ul>')	
		...
		self.response.write('<ul>')
		self.response.write('<li><h4>HTTP GET returns JSON object of all Resuse_item entities (in alphabetical order) with their properties.</h4></li>')
		self.response.write('<li><h4>HTTP POST creates a new Reuse_item entity and returns a JSON object of the entity and its properties</h4></li>')
		self.response.write('<ul>')
		self.response.write('<li>name - stored as a StringProperty, REQUIRED</li>')		
		self.response.write('</ul>')	
		self.response.write('<li><h4>HTTP PUT updates properties of an existing Reuse_item entity and returns a JSON object of the updated entity and its properties</h4></li>')
		self.response.write('<ul>')	
		self.response.write('<li>reuse_item_urlsafe_key - the urlsafe key of the Reuse_item entity to be updated, REQUIRED</li>')
		self.response.write('<li>name - stored as a StringProperty, REQUIRED</li>')	
		self.response.write('<li>updated_by - stored as a StringProperty</li>')
		self.response.write('</ul>')	
		self.response.write('</ul>')	

		# /reuse_item/delete/<reuse_item_urlsafe_key>
		self.response.write('<li><h2>')
		self.response.write(cgi.escape('/reuse_item/delete/<reuse_item_urlsafe_key>'))
		self.response.write('</h2></li>')
		self.response.write('<ul>')
